//! Hand-written reverse-mode VJP for the FSRS-5 BCE-loss gradient — a manual reverse-mode of the
//! exact forward in [`super::fsrs_v5::retention`]. Replaces the forward-mode `Dual<19>` autodiff for
//! the *training* gradient (forward-mode costs ~19× the value pass per op; reverse-mode is ~1 fwd +
//! ~1 bwd). Prediction keeps `Dual<0>`. f64 throughout (FSRS-5 is an f64 algo).
//!
//! FSRS-5 is FSRS-6 with a FIXED forgetting-curve decay (-0.5, no `w20`) and a simpler short-term
//! branch (`s·exp(w17·(w18+rating-3))`, no `s^-w19`/`max(·,1)`); success/fail/difficulty/init are
//! identical. So this mirrors `fsrs_v6_grad` minus the decay gradient and the short-term `s` power.
//!
//! ⚠ Manual VJP of a SPECIFIC forward — change the FSRS-5 formulas and you must re-derive the
//! backward here. Guarded by `fsrs5_analytic_grad_matches_forward_mode` (gated to `--features fp64`).
...
pub struct WConsts {
    s_min: f64,
    s_max: f64,
    factor: f64, // 0.9^(1/DECAY) - 1   (constant — DECAY fixed)
    exp_w8: f64,
    exp3w5: f64,
    initd4: f64,
    msd: f64, // exp(w17·w18)
...
pub fn wconsts(w: &[f64], s_min: f64, s_max: f64) -> WConsts {
    ...
    WConsts {
        s_min,
        s_max,
        factor: 0.9f64.powf(1.0 / DECAY) - 1.0,
        exp_w8: w[8].exp(),
        exp3w5,
...
// fc(t, s) = (1 + factor·t/s)^DECAY  (DECAY fixed ⇒ no parameter gradient from the curve).
...
/// VJP of the curve. Returns g_s (no parameter grads — decay is fixed).
#[inline]
fn curve_bwd(c: &CurveCache, t: f64, s: f64, g_out: f64, wc: &WConsts) -> f64 {
    let g_base = g_out * DECAY * c.out / c.base; // ∂out/∂base
    g_base * wc.factor * (-t / (s * s)) // ∂(t/s)/∂s
}
...
pub struct StepCache {
    ...
    delta_d: f64,
    nd0: f64,
    nd_pre: f64, // initd_pre for INIT
    cr: CurveCache,
    r: f64,
    e17: f64, // SHORT: exp(w17·(w18+rating-3))
    spb: f64, // SUCCESS
    rrb: f64,
    term: f64,
    hard: f64,
...
fn step_fwd(w: &[f64], s: f64, d: f64, dt: f64, rating: f64, wc: &WConsts) -> ((f64, f64), StepCache) {
    ...
    let ns = if short_term {
        c.branch = SHORT;
        let e17 = (w[17] * (w[18] + (rating - 3.0))).exp();
        c.e17 = e17;
        s * e17
    } else {
...
fn step_bwd(w: &[f64], c: &StepCache, g_s_new: f64, g_d_new: f64, wc: &WConsts, gw: &mut [f64]) -> (f64, f64) {
    ...
    match c.branch {
        SHORT => {
            // ns = s·e17 ; e17 = exp(w17·(w18+rating-3))
            g_s += g_ns * c.e17;
            let g_e17 = g_ns * c.s;
            gw[17] += g_e17 * c.e17 * (w[18] + (c.rating - 3.0));
            gw[18] += g_e17 * c.e17 * w[17];
        }
        SUCCESS => {
            g_s += g_ns * (1.0 + c.term);
            let g_term = g_ns * c.s;
...
mod tests {
    ...
    use crate::models::fsrs_v5::{retention_dual, INIT_W};
    ...
    #[cfg(feature = "fp64")]
    #[test]
    fn fsrs5_analytic_grad_matches_forward_mode() {
